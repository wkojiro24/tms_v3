class VehiclesController < ApplicationController
  ...
  def schedule
    ...
    @schedule_group_id = params[:vehicle_group].presence
    ...
    # VehicleGroupでフィルタリング
    if @schedule_group_id.present?
      @selected_group = VehicleGroup.find_by(id: @schedule_group_id)
      if @selected_group
        group_codes = @selected_group.normalized_vehicle_codes
        scope = scope.where(call_sign: group_codes)
                     .or(scope.where(registration_number: group_codes))
      end
    end
