module VehicleFinancialsHelper
  ...
  def vehicle_financial_value(label, value, cell_state: nil)
    # エラー状態の場合
    if cell_state == "error"
      return content_tag(:span, "⚠", class: "text-warning", title: "計算エラー (#DIV/0! 等)")
    end

    # 空白状態の場合
    if cell_state == "blank"
      return content_tag(:span, "-", class: "text-muted", title: "データなし")
    end
