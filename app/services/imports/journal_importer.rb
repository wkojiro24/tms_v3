...
module Imports
  class JournalImporter
    # ヘッダー行は自動検出するため、定数は最大検索範囲として使用
    MAX_HEADER_SEARCH_ROW = 15
    ...
    def call
      ...
      entry_cache = {}
      stats = { total_rows: 0, skipped_empty: 0, skipped_no_attrs: 0, processed: 0 }

      Rails.logger.info "[JournalImporter] Starting import: #{source_file_name}"
      Rails.logger.info "[JournalImporter] Sheets found: #{spreadsheet.sheets.inspect}"
      ...
      spreadsheet.sheets.each do |sheet_name|
        ...
        unless header_rows
          Rails.logger.warn "[JournalImporter] Sheet '#{sheet_name}': Header row not detected, skipping"
          next
        end

        headers = build_headers(sheet, header_rows[:parent], header_rows[:child])

        if headers.compact.blank?
          Rails.logger.warn "[JournalImporter] Sheet '#{sheet_name}': No valid headers found, skipping"
          next
        end

        # ヘッダーの主要カラムをログ出力
        key_headers = headers.each_with_index.select { |h, _| h.present? && h.to_s.include?("金額") }.map { |h, i| "#{i}:#{h}" }
        Rails.logger.info "[JournalImporter] Sheet '#{sheet_name}': Header at row #{header_rows[:parent]}, 金額 columns: #{key_headers.join(', ')}"
        ...
        start_row.upto(sheet.last_row) do |row_index|
          ...
          unless row_attrs
            ...
            # 最初の5件だけログ出力
            if sheet_skipped <= 5
              Rails.logger.debug "[JournalImporter] Row #{row_index} skipped: date=#{values[0].inspect}, debit_amt=#{values[14].inspect}, credit_amt=#{values[22].inspect}"
            end
            next
          end
        ...
        Rails.logger.info "[JournalImporter] Sheet '#{sheet_name}': #{sheet_rows} rows, #{sheet_skipped} skipped"
      end
      ...
      Rails.logger.info "[JournalImporter] Import complete: total=#{stats[:total_rows]}, empty=#{stats[:skipped_empty]}, no_attrs=#{stats[:skipped_no_attrs]}, processed=#{stats[:processed]}, entries=#{entry_cache.size}"

      if entry_cache.empty? && stats[:total_rows] > 0
        Rails.logger.error "[JournalImporter] WARNING: No entries created from #{stats[:total_rows]} rows. Check header format compatibility."
      end
    ...
    def build_headers(sheet, parent_row_num, child_row_num)
      parent_row = safe_row(sheet, parent_row_num)
      child_row = safe_row(sheet, child_row_num)
    ...
    # ヘッダー行を自動検出する
    # 「日付」と「借方」を含む行を親ヘッダー行とし、次の行を子ヘッダー行とする
    def detect_header_rows(sheet)
      (1..MAX_HEADER_SEARCH_ROW).each do |row_num|
        row = safe_row(sheet, row_num)
        row_text = row.map(&:to_s).join(" ")

        # 「日付」と「借方」の両方を含む行がヘッダー行
        if row_text.include?("日付") && row_text.include?("借方")
          Rails.logger.info "[JournalImporter] Header detected at row #{row_num}"
          return { parent: row_num, child: row_num + 1 }
        end
      end

      # 見つからなかった場合はnil
      Rails.logger.warn "[JournalImporter] Header row not found in first #{MAX_HEADER_SEARCH_ROW} rows"
      nil
    end
